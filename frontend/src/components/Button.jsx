...
const Button = ({ type, children, ...rest }) => {
  if (type === 'danger')
    return (
      <button
        className="rounded-md  flex-[2_1_0%] px-3.5 py-2 m-1 overflow-hidden relative group cursor-pointer border-2 font-medium border-red-600 text-red-600"
        {...rest}
      >
  ...
  if (type === 'normal')
    return (
      <button
        {...rest}
        className="rounded-md  flex-[2_1_0%] px-3.5 py-2 m-1 overflow-hidden relative group cursor-pointer border-2 font-medium border-purple-600 text-purple-600 mx-auto"
      >
